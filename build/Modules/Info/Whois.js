...
class Whois extends Command_1.Command {
    constructor() {
        super({
            name: 'whois',
            module: 'info',
    ...
    async execute(metis, ctx) {
        ...
        if (!member) {
            return this.error(ctx.channel, `Could not find user ${ctx.args.join(' ')}`);
        }
        ;
        ...
        let joinPos = (ctx.guild.members.filter((m) => !m.bot).sort((a, b) => a.joinedAt - b.joinedAt).map((m) => m.id).indexOf(member.id) + 1).toString();
        if (joinPos === undefined || member.bot) {
            joinPos = 'N/A';
        }
        if (joinPos === undefined || member.bot) {
            joinPos = 'N/A';
        ...
        if (member.nick) {
            data.embed.description = `${member.mention}\n**Nickname** - ${member.nick}`;
        }
        ...
        data.embed.fields.push({ name: 'Joined Server', value: `${joined}` }, { name: 'Account Created', value: `${createdAt}`, inline: true });
