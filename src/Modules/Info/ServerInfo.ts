...
class ServerInfo extends Command { 
    ...
    async execute(metis: MetisInterface, ctx: ICommandContext): Promise<any> {
        ...
        let owner: any = guild.members.get(guild.ownerID); 
        if (!owner === null) { 
            owner = 'Not cached'
        }
        ...
        let vLevel: any = guild.verificationLevel; 
        if (vLevel === 0) {vLevel = 'None'}
        if (vLevel === 1) {vLevel = 'Low'}
        if (vLevel === 2) {vLevel = 'Medium'}
        if (vLevel === 3) {vLevel = 'High'}
        if (vLevel === 4) {vLevel = 'Extreme'}

        let mfa: any = guild.mfaLevel; 
        if (mfa === 0) {mfa = 'Not Required'}
        if (mfa === 1) {mfa = 'Required'}

        let expl: any = guild.explicitContentFilter; 
        if (expl === 0) {expl = 'Off'} 
        if (expl === 1) {expl = 'On for people without roles'} 
        if (expl === 2) {expl = 'On for everyone'}

        let large: any = guild.large
        if (large) {large = 'Yes'} 
        else {large = 'No'}
