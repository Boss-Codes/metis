...
class RoleInfo extends Command { 
    ...
    async execute(metis: MetisInterface, ctx: ICommandContext): Promise<any> {
        if (!ctx.args[0]){return this.error(ctx.channel, 'Provide a role!')}

        let role = ctx.guild.roles.get(ctx.args[0]); 
        if (!role && ctx.msg.roleMentions && ctx.msg.roleMentions[0]) {role = ctx.guild.roles.get(ctx.msg.roleMentions[0])}
        if (!role) {role = ctx.guild.roles.find(r => r.name.toLowerCase().startsWith(ctx.args.join(' ').toLowerCase()))}
        if (!role || role.id === ctx.guild.id){return this.error(ctx.channel, 'Invalid role provided!')}
